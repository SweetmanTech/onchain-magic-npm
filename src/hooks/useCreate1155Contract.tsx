...
const useCreate1155Contract = () => {
  ...
  const signTransaction = async (args) => {
    if (authenticated) {
      await sendTransaction(factoryAddress, chainId, abi, "createContract", args)
      const response = await getCreated1155DropsByAddress(connectedWallet, chainId)
      push(`/create/story/${response[0]}`)
    }
  }

  const createContract = async () => {
    try {
      const ipfs = await store(getZoraBlob(direccionDePago), contractName, "", direccionDePago)
